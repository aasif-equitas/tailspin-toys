...
- Define and document a `Props` interface in frontmatter for every reusable component. Describe each prop's purpose and any constraints in TSDoc comments.
...
- Comment why a component uses a non-obvious structure or workaround; do not comment markup by paraphrasing it.
- Keep component API documentation current when adding, removing, or changing props.
